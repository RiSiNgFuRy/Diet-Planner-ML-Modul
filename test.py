...
# parse food database
data = pd.read_csv('C:/Users/hp/Desktop/diet-planner/food.csv')

# parse nutritional distribution (trained data)
datafin = pd.read_csv('C:/Users/hp/Desktop/diet-planner/nutrition_distriution.csv')
...
def diet_planner():
    ...
    # get ideal intake
    if gender==0:
        bmr_ideal = (9.99 * weight) + (6.25 * height) - (5 * age) + 5
    elif gender==1:
        bmr_ideal = (9.99 * weight) + (6.25 * height) - (5 * age) - 161
    ...
    bmr=calorie_d+fat_d+protein_d+carbs_d+fibre_d+sugar_d
    ...
    #         Target Protein: {protein_d:.2f}kCal\n \
...
def get_meals(goal_cat, meal, bmr, param_size, goal, meal_data):
    ...
    meal_proteins=meal_data["Proteins"].tolist()
